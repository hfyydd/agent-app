      ...
      switch (toolCall.toolName) {
        ...
        case 'executeSql': {
          try {
            ...
            const filteredResults = results.map((result: { fields: any[]; rows: any[] }) => {
              const vectorFields = result.fields.filter(
                (field: { dataTypeID: any }) => field.dataTypeID === vectorDataTypeId
              )

              return {
                ...result,
                rows: result.rows.map((row: { [s: string]: unknown } | ArrayLike<unknown>) =>
                  Object.entries(row).reduce(
                    (merged, [key, value]) => ({
                      ...merged,
                      [key]: vectorFields.some((field: { name: string }) => field.name === key)
                        ? `[${JSON.parse(String(value)).slice(0, 3).join(',')},...]`  
                        : value,
                    }),
                    {}
                  )
